...
router.get('/:email/:number', async (req, res) => {
  var today = new Date();
  today.setDate(today.getDate() - req.params.number);
    Product.find({ email: req.params.email, added:true, creationDate: {$gte: today} }).
        then(data => {
            res.send(data)
        }).catch(error => res.status(500).send(error))
})

router.get('/notAdded/:email/:number', async (req, res) => {
  var today = new Date();
  today.setDate(today.getDate() - req.params.number);
    Product.find({ email: req.params.email, added: false, creationDate: {$gte: today}  }).
        then(data => {
            res.send(data)
        }).catch(error => res.status(500).send(error))
